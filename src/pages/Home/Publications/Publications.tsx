...
export const Publications: React.FC = () => {
    ...
    const [prevActiveCategory, setPrevActiveCategory] = useState<Category>(Category.Events);
    ...
    useEffect(() => {
        const newDirection = categoryValues.indexOf(activeCategory) > categoryValues.indexOf(prevActiveCategory) ? 1 : -1;
    ...
    useEffect(() => {
        const updateUnderline = () => {
            const activeTitleIndex = publicationCategory.findIndex(cate => cate.title === activeCategory);
            if (activeTitleIndex !== -1 && titleRefs.current[activeTitleIndex]) {
                const { width, left } = titleRefs.current[activeTitleIndex]!.getBoundingClientRect();
