    ...
    return (
        <footer>
                    ...
                    {footerData.map((col, index) => (
                        <div key={index} className='flex flex-col'>
                            ...
                            {col.subs.map((sub, index) => (
                                <div key={index} className='py-1 flex flex-row items-center text-base'>
                                    <Icon icon="weui:arrow-outlined" className="h-5 w-5 mr-2 text-white" />
                                    {sub.label}
                                </div>
                            ))}
                        </div>
            ...
            <div className="bg-black h-[50px] flex px-8 items-center">
                <p className='text-white text-sm'>Copyright © 2024 Hong Kong Council for Testing and Certification. All Rights Reserved</p>
            </div>
        </footer>
    );
};
